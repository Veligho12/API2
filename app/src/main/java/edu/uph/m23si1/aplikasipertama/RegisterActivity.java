...
import edu.uph.m23si1.aplikasipertama.api.RegenciesResponse;

import edu.uph.m23si1.aplikasipertama.model.KabupatenKota;
...
public class RegisterActivity extends AppCompatActivity {
    Button btnRegister;
    EditText edtName;
    Spinner sprProvinsi, sprKabupatenKota;
    ...
    //NEW
    List<String> namaKabupatenKota = new ArrayList<>();
    ArrayAdapter<String> kabupatenKotaAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //NEW
        sprKabupatenKota= findViewById(R.id.sprKabupatenKota);
        kabupatenKotaAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, namaKabupatenKota);
        kabupatenKotaAdapter.setDropDownViewResource(android.R.layout.simple_spinner_item);
        sprKabupatenKota.setAdapter(kabupatenKotaAdapter);
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    sprProvinsi.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                        ...
                        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                            ...
                            //NEW
                            String kodeProvinsi = selected.getCode();
                            Toast.makeText(RegisterActivity.this,
                                    "Kode : " + selected.getCode() + " Nama Provinsi : " + selected.getName(),
                                    Toast.LENGTH_LONG).show();

                            apiService.getRegencies(kodeProvinsi).enqueue(new Callback<RegenciesResponse>() {
                                @Override
                                public void onResponse(Call<RegenciesResponse> call, Response<RegenciesResponse> response) {
                                    if (response.isSuccessful() && response.body() != null) {
                                        namaKabupatenKota.clear();
                                        for (KabupatenKota k : response.body().getData()) {
                                            namaKabupatenKota.add(k.getName());
                                        }
                                        kabupatenKotaAdapter.notifyDataSetChanged();
                                    }
                                }

                                @Override
                                public void onFailure(Call<RegenciesResponse> call, Throwable t) {
                                    Toast.makeText(RegisterActivity.this,
                                            "Gagal ambil kabupaten: " + t.getMessage(),
                                            Toast.LENGTH_LONG).show();
                                }
